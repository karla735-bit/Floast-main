...
export async function getUnits(propId) {
  try {
    const response = await fetch(`http://localhost:8080/api/properties/${propId}/units`);

    // Si no hay unidades creadas aún, devolvemos un arreglo vacío
...
export async function updateUnit(propId, unitId, data) {
  try {
    // Apuntamos al UnitController que creamos en Spring Boot
    const API_URL = `http://localhost:8080/api/properties/${propId}/units/${unitId}`;
    ...
    const response = await fetch(API_URL, {
      ...
      // Convertimos los datos del inquilino y la unidad a formato JSON
      body: JSON.stringify(data)
    });

    if (!response.ok) {
      const errText = await response.text();
      throw new Error(errText || "Error al actualizar la unidad en el servidor.");
    }

    // Retornamos la respuesta exitosa del backend
    return await response.json();
